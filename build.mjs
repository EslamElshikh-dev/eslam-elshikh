...
function googleBusinessAuditPage() {
  ...
  const faq = [
    ...
    ["هل تحفظ الأداة اسم النشاط أو الرابط؟", "لا تحفظ الأداة المدخلات في قاعدة بيانات. عند تفعيل البحث المباشر يُرسل استعلامك إلى Google Places للحصول على البيانات العامة، وعند اختيار WhatsApp تراجع الرسالة قبل إرسالها."],
    ["هل يفحص الموقع تقنيًا؟", "يفحص وجود رابط الموقع واتساقه فقط. فحص السرعة والجوال والفهرسة يحتاج زحفًا منفصلًا ويُراجع في التشخيص الاحترافي."],
    ["هل يمكن استخدام رابط Google Maps؟", "نعم للروابط العامة التابعة لـGoogle. إذا تعذر استخراج اسم النشاط من رابط مختصر، اكتب اسم النشاط مع المدينة أو استخدم الفحص الإرشادي."]
  ];
  const auditService = {
    "@type": "Service",
    "@id": `${absolute(path)}#service`,
    name: "فحص أولي مجاني لملف Google التجاري",
    description: "فحص أولي للبيانات العامة واكتمال أساسيات الملف التجاري مع درجة جاهزية وأولويات، دون وعد بترتيب أو قرار من Google.",
    provider: { "@id": `${site.url}/#professional-service` },
  ...
  const body = `${innerHero({ eyebrowText: "أداة مجانية لأصحاب الأنشطة", title: "فحص ملف Google التجاري خلال 60 ثانية", lead: "اكتب اسم نشاطك مع المدينة أو الصق رابط Google Maps العام. ستحصل على قراءة أولية شفافة من 100 وثلاث أولويات للتحسين، ثم تختار إن كنت تريد مراجعة احترافية.", path, crumbs: [{ name: "فحص ملف Google", path }], aside: `<span class="aside-kicker">Google Business Profile Audit</span><strong>بيانات عامة فقط، بلا كلمات مرور</strong><p>لا تطلب الأداة الدخول إلى حسابك، ولا تدّعي أنها تتنبأ بترتيب Google أو قرار التحقق.</p>` })}
<section class="section-pad growth-shell"><div class="container growth-grid"><div class="growth-panel reveal" data-gbp-audit role="form" aria-labelledby="audit-title"><h2 id="audit-title">ابدأ الفحص الأولي</h2><p>لأفضل تطابق، اكتب الاسم كما يظهر على Google ثم المدينة. يمكنك إضافة موقعك ورقمك المتوقعين لفحص اتساق NAP.</p><div class="audit-fields"><label><span>اسم النشاط والمدينة أو رابط Google Maps</span><input type="text" maxlength="220" autocomplete="organization" placeholder="مثال: اسم النشاط، الرياض" data-audit-query required></label><div class="audit-optional"><label><span>موقعك المتوقع — اختياري</span><input type="url" maxlength="240" inputmode="url" autocomplete="url" placeholder="https://example.com" data-audit-website></label><label><span>هاتفك المتوقع — اختياري</span><input type="tel" maxlength="30" inputmode="tel" autocomplete="tel" placeholder="05xxxxxxxx" data-audit-phone></label></div><div class="audit-action-row"><button class="button" type="button" data-audit-submit>افحص الملف الآن ${icon("search", "button-icon")}</button><span>لا ترسل كلمات مرور أو رموز تحقق.</span></div><p class="audit-status" role="status" aria-live="polite" data-audit-status></p></div><div class="audit-choices" data-audit-choices hidden></div><div class="manual-audit" data-manual-audit hidden><h2>الفحص الإرشادي داخل جهازك</h2><p>حدد ما يمكنك تأكيده من ملفك العام. لا تُرسل هذه الاختيارات إلى الخادم.</p><div class="manual-checks">${manualChecks.map(([name, weight, label, dataLabel]) => `<label><input type="checkbox" name="${name}" value="${weight}" data-label="${esc(dataLabel)}"><span>${esc(label)} <strong dir="ltr">+${weight}</strong></span></label>`).join("")}</div><button class="button button-ghost" type="button" data-manual-calculate>احسب الدرجة الإرشادية</button></div><section class="audit-result" data-audit-results hidden><div class="audit-result-head"><div class="audit-score"><strong data-score>0</strong><span>من 100</span></div><div><span class="eyebrow"><span></span><b data-score-label>النتيجة</b></span><h2 data-result-name></h2><p data-result-meta></p><p data-result-rating></p></div></div><div class="audit-result-grid"><div class="audit-result-card"><h3>ما ظهر في الفحص</h3><ul class="audit-checks" data-audit-checks></ul></div><div class="audit-result-card"><h3>أول ثلاث أولويات</h3><ol class="audit-priorities" data-audit-priorities></ol></div></div><div class="audit-result-actions"><a class="button" href="${site.whatsapp}" target="_blank" rel="noopener" data-audit-whatsapp>أرسل النتيجة للمراجعة ${icon("whatsapp", "button-icon")}</a><a class="button button-ghost" href="#" target="_blank" rel="noopener" data-result-maps hidden>فتح الملف العام ${icon("external", "button-icon")}</a></div></section></div><aside class="growth-panel reveal"><h2>ماذا تقيس الدرجة؟</h2>${checkList(["وضوح الاسم والفئة والموقع الجغرافي", "الهاتف والموقع وساعات الحضور", "حد أولي من المراجعات والصور العامة", "اتساق الهاتف والنطاق عند إدخالهما"])}<div class="growth-note">${icon("shield")}<p><strong>حدود الفحص:</strong> لا يرى الأداة لوحة الإدارة أو حالة التحقق أو التعديلات المعلقة أو المنافسين. النتيجة تشخيص أولي وليست اعتمادًا من Google.</p></div><div class="section-action">${button("/book/", "احجز مراجعة احترافية", "button-ghost")}${button("/local-visibility-dashboard/", "افتح لوحة القياس الخاصة", "button-ghost")}</div></aside></div></section>
<section class="section-pad muted-section"><div class="container faq-grid"><div class="faq-intro reveal">${eyebrow("قبل استخدام النتيجة")}<h2>درجة مفهومة وليست صندوقًا أسود</h2><p>كل نقطة مرتبطة بإشارة عامة تستطيع مراجعتها. أي قرار حساس مثل تغيير الاسم أو العنوان أو الفئة يبدأ من واقع النشاط وسياسات Google.</p></div>${faqBlock(faq)}</div></section>`;
  return page({ title: "فحص ملف Google التجاري مجانًا خلال 60 ثانية", description: "افحص ملف Google التجاري مجانًا: درجة جاهزية أولية من 100، كشف نقص البيانات العامة واتساق الهاتف والموقع، وثلاث أولويات لتحسين الملف.", path, active: "google", body, stylesheets: [`/assets/css/google-growth.css?v=${version}`], pageScripts: [`/assets/js/google-audit.js?v=${version}`], keywords: ["فحص ملف جوجل التجاري", "تحليل خرائط جوجل", "Google Business Profile audit", "تحسين ملف جوجل"], schema: [auditService, faqSchema(faq), breadcrumbSchema([{ name: "الرئيسية", path: "/" }, { name: "فحص ملف Google", path }])] });
}
...
function englishGoogleBusinessAuditPage() {
  ...
  const body = `${innerHero({ eyebrowText: "Free Google Business Profile tool", title: "Run an initial profile readiness audit", lead: "The live Arabic tool checks public profile foundations and prepares three priorities. An English interface is being completed; you can use the same privacy-safe workflow now.", path, language: "en", crumbs: [{ name: "Google Business Profile audit", path }], aside: `<span class="aside-kicker">60-second audit</span><strong>No passwords or account access</strong><p>The score is a transparent readiness checklist, not a ranking forecast or a Google decision.</p>` })}<section class="section-pad"><div class="container growth-grid"><div class="growth-panel reveal"><h2>Use the live audit</h2><p>Enter the public business name and city or a public Google Maps URL. The Arabic tool also accepts your expected website and phone for a basic consistency check.</p>${button("/google-business-profile-audit/", "Open the live audit")}</div><aside class="growth-panel reveal"><h2>What it covers</h2>${checkList(["Public identity, category, location, and contact fields", "Published hours, review baseline, and public photos", "Optional website and phone consistency", "Three clear priorities and a WhatsApp review handoff"])}<div class="growth-note">${icon("shield")}<p>The tool does not access the management console, verification state, pending edits, or competitors. A professional review is still needed before sensitive changes.</p></div></aside></div></section>`;
  return page({ title: "Free Google Business Profile Audit", description: "Run a privacy-aware initial Google Business Profile readiness audit and identify three practical priorities before a professional review.", path, active: "google", body, lang: "en", stylesheets: [`/assets/css/google-growth.css?v=${version}`], schema: [breadcrumbSchema([{ name: "Home", path: "/en/" }, { name: "Google Business Profile audit", path }])] });
}
