...
function businessMapSection() {
  return `<section class="section-pad business-map-section" id="google-business-map"><div class="container"><div class="section-heading business-map-heading reveal">${eyebrow("الملف التجاري على Google")}<h2>المهندس إسلام الشيخ على خرائط Google</h2><p>افتح الملف التجاري مباشرة لمراجعة بيانات النشاط والخدمات وطرق التواصل. تُقدَّم الخدمات داخل نطاق مدينة الرياض وعن بُعد وفق طبيعة المشروع، ولا يوجد مقر مخصص لاستقبال العملاء.</p></div><div class="business-map-grid"><div class="business-map-frame reveal"><iframe src="${esc(site.googleMapsEmbed)}" title="خريطة الملف التجاري للمهندس إسلام الشيخ على Google Maps" loading="lazy" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><aside class="business-map-card reveal"><span class="map-profile-badge">${icon("google")} Google Business Profile</span><h3>${esc(site.brandName)}</h3><p>خدمات تقنية واستشارية في تطوير المواقع والأمن السيبراني ومنتجات Google والسيو والإعلانات الرقمية.</p><dl class="business-map-details"><div><dt>${icon("pin")}نطاق الخدمة</dt><dd>${esc(site.city)}، ${esc(site.country)}</dd></div><div><dt>${icon("phone")}الهاتف</dt><dd><a href="tel:${site.phone}" dir="ltr">${esc(site.phoneDisplay)}</a></dd></div><div><dt>${icon("clock")}استقبال الطلبات</dt><dd>على مدار الساعة</dd></div></dl><div class="business-map-actions">${button(site.googleMapsProfile, "فتح الملف على خرائط Google", "", true)}${button(`${site.whatsapp}?text=${encodeURIComponent("مرحبًا م. إسلام، وصلت إليك من موقعك وأرغب في مناقشة خدمة.")}`, "تواصل عبر WhatsApp", "button-ghost", true)}</div></aside></div></div></section>`;
}
...
function serviceDetailPage(service) {
  ...
  const relevantBusinessMap = service.slug === "google-business-profile" ? businessMapSection() : "";
